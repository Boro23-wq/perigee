...
func CreateRecipe(c *gin.Context) {
	...
	err = db.Pool.QueryRow(c.Request.Context(),
		`INSERT INTO public.recipes
		   (creator_id, name, total_calories, protein, carbs, fat, servings, ingredients)
		 VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
		 RETURNING id, creator_id, name, total_calories, protein, carbs, fat, servings, ingredients, share_token, created_at`,
		userID, req.Name, req.TotalCalories, req.Protein, req.Carbs, req.Fat, req.Servings, string(ingredientsJSON),
	).Scan(&r.ID, &r.CreatorID, &r.Name, &r.TotalCalories, &r.Protein, &r.Carbs, &r.Fat,
...
func UpdateRecipe(c *gin.Context) {
	...
	err = db.Pool.QueryRow(c.Request.Context(),
		`UPDATE public.recipes
		 SET name = $1, total_calories = $2, protein = $3, carbs = $4, fat = $5,
		     servings = $6, ingredients = $7
		 WHERE id = $8 AND creator_id = $9
		 RETURNING id, creator_id, name, total_calories, protein, carbs, fat, servings, ingredients, share_token, created_at`,
		req.Name, req.TotalCalories, req.Protein, req.Carbs, req.Fat, req.Servings, string(ingredientsJSON),
		id, userID,
	).Scan(&r.ID, &r.CreatorID, &r.Name, &r.TotalCalories, &r.Protein, &r.Carbs, &r.Fat,
		&r.Servings, &rawIngredients, &r.ShareToken, &r.CreatedAt)
...
func GetRecipes(c *gin.Context) {
	...
	rows, err := db.Pool.Query(c.Request.Context(),
		`SELECT r.id, r.creator_id, r.name, r.total_calories, r.protein, r.carbs, r.fat,
	...
	for rows.Next() {
		...
		if err := rows.Scan(&r.ID, &r.CreatorID, &r.Name, &r.TotalCalories, &r.Protein, &r.Carbs,
			&r.Fat, &r.Servings, &rawIngredients, &r.ShareToken, &r.CreatedAt, &r.Mine); err != nil {
...
func GetSharedRecipe(c *gin.Context) {
	...
	var rawIngredients []byte
	err := db.Pool.QueryRow(c.Request.Context(),
		`SELECT id, creator_id, name, total_calories, protein, carbs, fat, servings, ingredients, created_at
...
func LogRecipeMeal(c *gin.Context) {
	...
	var protein, carbs, fat, servings float64
	err = db.Pool.QueryRow(c.Request.Context(),
		`SELECT r.name, r.total_calories, r.protein, r.carbs, r.fat, r.servings
		 FROM public.recipes r
		 WHERE r.id = $1 AND (
		   r.creator_id = $2
		...
		recipeID, userID,
	).Scan(&name, &totalCalories, &protein, &carbs, &fat, &servings)
	if err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": "recipe not found"})
		return
	}
	...
	row := db.Pool.QueryRow(c.Request.Context(),
		`INSERT INTO public.food_logs
		   (user_id, date, meal_type, source, recipe_id, name, calories, protein, carbs, fat)
		 VALUES ($1, $2, $3, 'recipe', $4, $5, $6, $7, $8, $9)
		 RETURNING `+mealColumns,
		userID, req.Date, req.MealType, recipeID, name,
		int(float64(totalCalories)/servings), protein/servings, carbs/servings, fat/servings,
	)
	m, err := scanMeal(row)
	if err != nil {
